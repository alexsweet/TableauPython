...
# open S3 Connection
def getS3():
    session = Session(aws_access_key_id=os.environ.get('AWSACCESSKEYID'),
...
def getS3Bucket(s3, bucket):
    return s3.Bucket(name=bucket)

# load file to S3
def uploadFileToS3(s3, file, bucket, filepath):
    s3.meta.client.upload_file(file, bucket, filepath)

# write temp file and send to S3
def createTempFileUpload(s3, bucketName, bucketDir, fileName, dataStr):
    with gzip.open(fileName, "w") as f:
        f.write(bytearray(dataStr, "utf8"))
        f.close()
        uploadFileToS3(s3, fileName, bucketName, bucketDir + "/" + fileName)
        os.remove(fileName)
...
def get_jobs(server, auth_token, site_id, date=None):
    ...
    # Builds the request
    url = server + "/api/{0}/sites/{1}/jobs?filter=jobType:eq:refresh_extracts".format(VERSION, site_id)
    if date is not None:
...
def get_datasource_by_job_id(server, auth_token, site_id, job_id):
    ...
    """
    url = server + "/api/{0}/sites/{1}/jobs/{2}".format(VERSION, site_id, job_id)
...
if __name__ == "__main__":
    ...
    personalAccessTokenSecret = os.environ.get('TABLEAU_TOKEN')
    bucketName = 'BucketName'
    bucketDir = 'Tableau'

    # get s3 files in bucket useing a filter for the folder provided
    s3 = getS3()

    # sign into tableau rest api
    auth_token, site_id, user_id = sign_in(server, personalAccessTokenName, personalAccessTokenSecret)

    # get json file and format
    jobs_json = get_jobs(server, auth_token, site_id, fourDaysAgo)
    json_format = json.dumps(jobs_json, sort_keys=False, indent=4)

    # upload file to s3
    createTempFileUpload(s3, bucketName, bucketDir, 'jobs.json.gz', json_format)
